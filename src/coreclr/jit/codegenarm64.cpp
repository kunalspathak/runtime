                                   ...
                                   bool*     pTmpRegIsZero)
{
    ...
    if (spDelta != 0)
    {
        ...
        if ((spOffset == 0) && (spDelta >= -512))
        {
            ...
            GetEmitter()->emitIns_R_R_R_I(INS_stp, EA_PTRSIZE, reg1, reg2, REG_SPBASE, spDelta, INS_OPTS_PRE_INDEX); // need to use EA_16BYTE for sve registers
    ...
    if (needToSaveRegs)
    {
        ...
        if (emitter::isFloatReg(reg1) && (reg1 >= REG_V16))
        {

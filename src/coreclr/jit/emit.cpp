...
void emitter::emitCheckAlignFitInCurIG(unsigned nAlignInstr)
{
    ...
    // Or current IG doesn't have a removable jump.
    // If neither of condition match, force create a new IG.
    if ((emitCurIGfreeNext + instrDescSize >= emitCurIGfreeEndp) || ((emitCurIG->igFlags & IGF_HAS_REMOVABLE_JMP) != 0))
    {
        emitForceNewIG = true;
    }
}
